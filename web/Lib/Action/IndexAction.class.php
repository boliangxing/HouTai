...
class IndexAction extends BaseAction {
    ...
    public function control(){
    	...
    	if($Type==3){
			...
			$procedure = mssql_init("PHP_GetNewlyControl",$conn); 
	...
	public function kcList(){
    	$conn=mssql_connect($GLOBALS['DB_KZ']['DB_HOST'],$GLOBALS['DB_KZ']['DB_USER'],$GLOBALS['DB_KZ']['DB_PWD']); 
			...
			$procedure = mssql_init("GSP_GR_GetKcList",$conn); 
			$resource = mssql_execute($procedure,false);
			while($row = mssql_fetch_assoc($resource)){
				$rs[] = $row;
			}		
    	...
    	for($i=0;$i<count($rs);$i++){
			$rs[$i]['no'] = $i+1;
			$rs[$i]['ServerName'] = iconv("GBK","UTF-8",$rs[$i]['GameName']).'--'.iconv("GBK","UTF-8",$rs[$i]['ServerName']);
			$rs[$i]['ScoreAfter'] = number_format($rs[$i]['ScoreAfter']);
			$rs[$i]['ScoreBefore'] = number_format($rs[$i]['ScoreBefore']);
			$rs[$i]['ChangeScore'] = number_format($rs[$i]['ChangeScore']);
		}
		//$this->assign('type',$Type);
    	//$this->assign('pageNum',$pageNum);
    	$this->assign('result',$rs);
